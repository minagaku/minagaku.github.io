  ...
  const twitterHeader = (
    <>
      ...
      <a href={`https://twitter.com/${t.TweetedBy}`} className="a2">
        {t.UserDetails.FullName} ({t.TweetedBy})
      </a>
    </>
  )
    ...
    <>
      ...
      <div className="home">
        <div className="kuguri-area">
          ...
          <div className="text">
            <Link to={`/${prefixes.value.students}/クグリ・ストラトス`} className="name">
              [クグリ・ストラトス]
            </Link>
            やぁ、喫茶マッターホルンへようこそ！今日はいい食材が入っているぞ。
            <br />
            <br />
            <a href="https://w.atwiki.jp/ragadoon/pages/1276.html">今週の活動方針</a>はもう投稿したかな？ 余裕をもって回答しよう（自戒）！
          </div>
        </div>
        <div>
        ...
        <div className="minagaku-tl">
          ...
          {tweets.value.map(t => (
            <div className="tweet" key={t.TweetId}>
              <div className="tweet-header">{findStudent(prefixes.value.students, students.value, t)}</div>
              ...
              <a className="time" href={`https://twitter.com/${t.TweetedBy}/status/${t.TweetId}`}>
                <time>{t.CreatedAtIso}</time>
              </a>
            </div>
          ))}
        </div>
